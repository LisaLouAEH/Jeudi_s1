=begin
Tu vas stocker tout ces handles dans un array et 
tu vas dévoir coder un programme ruby qui repondra aux questions.
=end
...
def q1
    ...
    puts "#{@handle.size} Journalistes"
end

#Q2
def q2
    puts "Combien d'handle contiennent un numéro ?"
    #methode avec compteur
    puts "methode avec compteur i dans un while"
    print "= "
    i = 0
    sum = 0
    while i <= @handle.size
    ...
    puts sum
#methode avec grep : objet.grep(/condition recherchée/)
    puts "nbr d'id qui contient des chiffre #{@handle.grep(/[0-9]/).count}"
end

def q3 
# le ' i ' aprés la /condition/ du grep permet d'ignorer les ttt appliqué au caractères (ex: majuscule/minuscule/accent etc...)
    puts "Combien d'handle contiennent les 4 lettres du prénom <<Aude>> à la suite ?"
    puts "#{@handle.grep(/Aude/i).count} Handles"
end

def q4
#le ^@ isole le premier char de chaque élément pour en comparer la valeur avec le regexp
    puts "Combien commencent par une majuscule"
    puts "#{@handle.grep(/^@[A-Z]/).count} Handles"
end
...
def q6
#Combien de _ dans tous les pseudos confondus ? on veux pas 72 mais 75.
    puts "Combien de _ dans tous les pseudos confondus ?"
    puts " #{@handle.join.count("_")} caractere '_' ."
end

def q7
#trier le tableau par ordre alpha
    puts "trier le tableau"
    print @handle.sort(&:casecmp)
end
